...
public class RESTAPIController {
     ...
     *
     *@param id
     *        The id of the String to be edited
     * @param editedRecipe
     *            The valid Recipe to be saved.
     * @return ResponseEntity indicating success if the Recipe could be saved to
     *         the inventory, or an error if it could not be
     */
    @PutMapping(BASE_PATH + "/recipe/{id}")
    public ResponseEntity editRecipe(@PathVariable final String id, @RequestBody @Valid final Recipe editedRecipe) {
    	Recipe recipeToEdit = Application.getCoffeeMaker().getRecipeBook().findRecipe( id );
    	System.out.println(editedRecipe.getMilk());
    	if ( recipeToEdit == null ) {
            return new ResponseEntity( "Recipe with the name " + editedRecipe.getName() + " does not exists",
                    HttpStatus.CONFLICT );
        } 
    	Application.getCoffeeMaker().getRecipeBook().editRecipe(Application.getCoffeeMaker().getRecipeBook().getRecipes().indexOf(recipeToEdit), editedRecipe);
    	return new ResponseEntity<String>("{\"result\":\"success\"}", HttpStatus.OK);
    }
